...
namespace BSUIRScheduleDESK.Services
{
    ...
    {
        public static int CheckLessonsCount(IEnumerable<DailyLesson> lessons, Lesson lesson, DateTime givenDate, int weekNum)
        {
            int result = 0;
            List<DateTime> dates = DateService.GetDatesOfWeekByDate(givenDate);
            var list = lessons
                .Where(l => l.Lesson!.subject == lesson.subject
                    && l.Lesson!.lessonTypeAbbrev == lesson.lessonTypeAbbrev
                    && l.Lesson!.numSubgroup == lesson.numSubgroup
                    && l.Lesson!.weekNumber != null
                    && DateTime.TryParse(l.Lesson!.startLessonDate, out _)
                    && TimeOnly.TryParse(l.Lesson!.startLessonTime, out _))
                .OrderBy(l => DateTime.Parse(l.Lesson!.startLessonDate!));

            if (!list.Any())
                return result;

            DateTime firstLessonDate = DateTime.Parse(list.First().Lesson!.startLessonDate!);
            ...
            {
                list.Count(l => l.Lesson!.weekNumber!.Contains(1)),
                list.Count(l => l.Lesson!.weekNumber!.Contains(2)),
                list.Count(l => l.Lesson!.weekNumber!.Contains(3)),
                list.Count(l => l.Lesson!.weekNumber!.Contains(4))
            };

            // Getting the number of lessons for a given day
            result += list.Count(l => (int)l.Day + 1 == (int)givenDate.DayOfWeek
                                && TimeOnly.Parse(lesson.startLessonTime!) >= TimeOnly.Parse(l.Lesson!.startLessonTime!)
                                && l.Lesson!.weekNumber!.Contains(weekNum));
            ...
            result += list.Count(l => (int)l.Day + 1 < (int)givenDate.DayOfWeek
                                && l.Lesson!.weekNumber!.Contains(weekNum)
                                && DateTime.Parse(l.Lesson!.startLessonDate!) <= givenDate);
            ...
            result += list.Count(l => (int)l.Day + 1 <= (int)dates.Last().DayOfWeek
                                && l.Lesson!.weekNumber!.Contains(weekNum)
                                && DateTime.Parse(l.Lesson!.startLessonDate!) >= firstLessonDate);
